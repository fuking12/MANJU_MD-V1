    ...
    if (!searchData) {
      ...
      while (retries > 0) {
        try {
          ...
          console.log("API Response (Search):", searchResponse.data);
          searchData = searchResponse.data;
          break;
        } catch (error) {
          retries--;
          console.error(`API retry ${3 - retries} failed:`, error.message);
          if (retries === 0) throw new Error("අයිස් භාණ්ඩාගාරයෙන් anime තොරතුරු ලබා ගැනීම අසාර්ථකයි: " + error.message);
    ...
    const animeSelectionHandler = async (update) => {
      ...
      while (detailRetries > 0) {
        try {
          ...
          if (detailRetries === 0) {
            throw new Error(`Anime විස්තර ලබා ගැනීම අසාර්ථකයි: ${error.message}`);
          }
          await new Promise(resolve => setTimeout(resolve, 1000));
        }
      }

      // API response එක validate කිරීම
      if (!detailData || typeof detailData !== "object" || !detailData.status || !detailData.data) {
        console.log("No valid detail data:", detailData);
        throw new Error(`අයිස් භාණ්ඩාගාරයේ anime විස්තර නැත: ${JSON.stringify(detailData)}`);
      }
