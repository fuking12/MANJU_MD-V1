...
cmd({
  pattern: 'love ?(.*)',
  desc: 'AI GF/BF chat - Sinhala mode',
  category: 'ai',
  use: '.love හෙලෝ බැබි',
  filename: __filename
}, async (client, message, match, extras) => {
  const input = match?.[1] || message.quoted?.text;

  if (!input) {
    return await message.reply("මට කතා කරන්න උදව්වක් ඕනෙ. උදා: .love හෙලෝ බැබි");
  }
    ...
    const res = await fetch("https://api.chatanywhere.com.cn/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: "system",
            content: "You're a sweet, flirty, romantic Sinhala-speaking lover bot."
          },
          {
            role: "user",
            content: input
