...
// Cache for storing search results and API responses
const cache = new Map();
...
// Function to download and validate video file
async function downloadAndValidateVideo(url, retries = 2) {
  try {
    // Verify file type with HEAD request
    try {
    ...
    // Verify file size (should be at least 100KB)
...
// Function to check progress of video processing
...
// Function to fetch video download data
...
cmd(
  ...
  async (robin, mek, m, { from, q, reply }) => {
    try {
      ...
      // Prepare response message with quality options
      let desc = `
VIDEO DOWNLOADER 🎬
...
`;
      ...
      // Update reaction to show ready state
      await robin.sendMessage(from, { react: { text: "✅", key: mek.key } });
